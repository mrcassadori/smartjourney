...
- **`OperationDrawer`** (novo, separado do `ServiceRequestDrawer` antigo): protocolo/cliente/prioridade/status/responsável/SLA/aging/motivo/próxima ação + timeline; ação "Avançar status" percorre `OPERATION_STATUS_FLOW` (simulada, registra na timeline); "Ver cliente" e "Abrir chamado" reaproveitam navegação/handlers já existentes (`openClient`, `openTicketModal`).
