...
- **Fase 5** ✅ (validada, 0 erros, 9 fluxos de regressão) — três upgrades visuais adotados do mockup de referência, sem reabrir o modelo "preencher lacunas": **Distribuição da carteira** (barras atual × proposta por classe, sempre disponíveis — reusa `ProdImpactBar` — com marca de alvo sobreposta só quando há estratégia) na Carteira Proposta; **Resumo Financeiro rico** (`prodFinancialSummary`: taxa média ponderada — só calculada quando todos os itens negociáveis compartilham o mesmo indexador, mostra "Múltiplos indexadores" quando não — e prazo médio ponderado por `maturityDate`) na Revisão; **Divergências de taxa** (produtos com `rateUpdated`: taxa antiga riscada → nova, "Aceitar nova taxa" grava `rateAccepted`/atualiza o item, "Substituir ativo" remove e volta ao Explorar na mesma classe) também na Revisão, com linha destacada na tabela e item no Check final. Cache `20260814d`.
...
- **Fase 6** ✅ (validada, 0 erros, 10 fluxos de regressão) — **carrinho persistente da proposta**, a partir de um novo mockup do usuário mostrando dois casos de uso (1 produto vs. múltiplos produtos) com o catálogo real como base fixa da jornada, não a Explorar interna. Estado do carrinho (`{ clientId, simulationId, items }`) subiu para `app.jsx` (`cart`/`updateCart`/`clearCart`), sobrevivendo à ida-e-volta `products ⇄ proposta`. **Catálogo real vira o "voltar" da jornada**: Comparar (Tela 03) e "Continuar buscando" da Carteira (Tela 05) devolvem para a `ProductsPage` de verdade — não mais para a Explorar simplificada (Tela 02) — sempre que a jornada foi iniciada **âncorada no catálogo** (`catalogAnchored`, derivado do view inicial); a porta cliente-primeiro (Necessidades/Explorar) manteve sua navegação interna intacta. `ProductsPage` ganhou a barra **"Carteira em construção"** (`ProdCartBar`, visível quando o carrinho tem itens): mostra cliente/contagem/valor e permite continuar marcando produtos e clicando "Adicionar à carteira"/"Comparar" **sem reabrir o seletor de cliente** (só reabre se o carrinho ainda estiver vazio — evita um cliente "fantasma" sequestrar a próxima seleção). **Fluxo de 1 produto corrigido** (`singleFlow`): antes, ao configurar um único produto vindo do catálogo, "Adicionar à carteira" caía na Explorar por engano (bug real, não só de forma); agora vai direto para o Confirmar, igual ao caso de uso de 4 telas do mockup. **Bug de sincronização corrigido**: as 4 mutações de item (`addProducts`/`upsertItem`/`updateItem`/`removeItem`) sincronizavam o carrinho via `useEffect([items])` — mas handlers que adicionam um item **e navegam para fora no mesmo clique** (ex.: "Adicionar à carteira" do Comparar) desmontam o componente antes do efeito rodar, perdendo o item; agora cada mutação computa o próximo array e chama `onCartChange` **no mesmo tick**, antes de qualquer navegação. Cache `20260814e`.

- **Fase 7** ✅ (validada, 0 erros, 10 fluxos de regressão) — o Detalhe/Configuração do singleFlow (1 produto vindo do catálogo) virou **drawer** sobre o catálogo escurecido, em vez de página cheia, fechando a última diferença de forma com o mockup. Miolo do Detalhe (características, condições da operação, impacto na carteira) extraído para `ProdDetailBody` (compartilhado, prop `compact` empilha as colunas), reusado por dois wrappers: `ProdDetailView` (página cheia, porta cliente-primeiro, inalterada) e o novo `ProdConfigDrawer` (usa o `Drawer` compartilhado, fecha em Cancelar/Escape/clique fora → `onExit`, mesmo padrão de todo drawer do Portal). Como o backdrop do drawer intercepta cliques na sidebar até ser fechado (comportamento consistente com os outros drawers do app, não um bug), os testes de regressão que navegam logo depois de abrir o singleFlow precisaram de um `Escape` explícito antes do próximo passo. Cache `20260814f`.

- **Fase 8** ✅ (validada, 0 erros, 11 fluxos de regressão) — a partir de um mockup de referência anotado pelo usuário ("Ajuste": o modal `ProdClientPickerModal` separado estava incorreto — a escolha do cliente deve acontecer dentro do próprio slide-over), o fluxo de **1 ativo a partir do catálogo (row click)** deixou de navegar para `/proposta`: agora vive inteiramente em `ProductsPage.jsx`, sem sair da página. `ProductDetailDrawer` ganhou um **seletor de cliente inline** (`ProdInlineClientPicker` — busca por nome/CPF com sugestões, sem modal) logo abaixo do título; enquanto nenhum cliente é escolhido, só características (Nível 1) aparecem e "Adicionar à carteira" fica desabilitado (regra de negócio do mockup). Ao escolher, o **mesmo painel revela in-place** condições da operação e impacto na carteira via `ProdDetailBody` (reaproveitado de `ProductJourney.jsx`, escopo global compartilhado), e o fluxo de confirmação/envio (`ProdConfirmModal`, também reaproveitado) roda direto ali, terminando em Ordens. Novo `attachCartClient` em `app.jsx` (variante de `ensureCartFor` sem `navigate`) anexa o cliente ao carrinho sem navegação. **Escopo deliberadamente restrito**: o multi-select (checkboxes + "Comparar"/"Adicionar à carteira" para 2+ produtos) continua usando o modal — só o slide-over de 1 ativo mudou nesta fase; Comparar e Carrinho ficam para ajustes seguintes. Cache `20260815a`.

- **Fase 9** ✅ (validada, 0 erros, 11 fluxos de regressão) — a partir da análise do arquivo Figma de referência (nó "Jornada do produto"), 5 ajustes: **(1) mini-cartão do cliente** no drawer de 1 ativo — Conta Inter/Saldo/Perfil/Relacionamento, logo abaixo do seletor inline, assim que um cliente é escolhido; **(5) Confirmar Envio virou página cheia**, não mais modal (`ProdConfirmModal` removido, substituído por `ProdConfirmPage` — reaproveitado tanto pelo singleFlow quanto pela Revisar multi-item): título "Confirmar Envio", card de sucesso com Cliente/Conta/Valor Total e dois links de texto ("Ver recomendação enviada" | "Ir para central de ordens"); o envio agora acontece **direto** ao clicar "Adicionar à carteira"/"Enviar recomendação", sem um "tem certeza?" intermediário (a própria Carteira/Revisar já cumpre esse papel) — `submitRecommendation` ganhou um parâmetro opcional `itemsOverride` porque o singleFlow chama o envio no mesmo clique que adiciona o item, antes do estado `items` refletir a mudança; **(6) chip "Cliente: Nome (conta)"** persistente na topbar (`Shell.jsx`) enquanto há uma recomendação em andamento (carrinho com cliente anexado) — clicável, retoma a recomendação; **(2) taxa negociável sem slider** — `ProdRateInput` trocou o slider por "Taxa mínima/Taxa máxima" como rótulos de texto acima do campo e uma nota "Referência atual: X · Atualizado às HH:MM" abaixo, igual à referência; **(4) "Valor sugerido para aplicação"** — renomeado de "Valor da aplicação" (mesmo campo, editável, só a redação mudou). O carrinho deixou de ser encerrado dentro de `submitRecommendation` — agora só é limpo quando o usuário sai da tela de sucesso (clique em qualquer um dos dois links), pra o chip/tela de confirmação continuarem mostrando o cliente enquanto visíveis. Cache `20260816b`.

- **Fase 10** ✅ (validada, 0 erros, 12 fluxos de regressão) — a partir da análise do nó Figma "Comparar investimentos", o fluxo de **Comparar** (porta catálogo, checkboxes + botão "Comparar") fechou a mesma lacuna que a Fase 8 fechou para o slide-over de 1 ativo: **(1) cliente inline, sem modal** — `openPicker`/`ProdClientPickerModal` não abre mais para o modo "comparar"; o clique em "Comparar" entra direto na tela via novo `ProdCompareInline` (`ProductsPage.jsx`), que resolve o cliente com o mesmo `ProdInlineClientPicker` do drawer de 1 ativo (pré-preenchido se já houver carrinho em construção) antes de revelar a tabela — a porta modal (`ProdClientPickerModal`) seguiu existindo só para "Adicionar à carteira" direto (2+ produtos sem comparar). `ProdCompareView` (compartilhado, `ProductJourney.jsx`) ganhou: **(2)** checkbox por ativo comparado no cabeçalho da coluna (dá pra tirar um ativo da comparação sem sair da tela; todos vêm marcados por padrão) — prop `hideHeader` deixa o cabeçalho "Comparar investimentos"/Voltar a cargo de quem envolve a tabela (o `ProdCompareInline` tem o seu, com o seletor de cliente entre o título e a tabela); **(3)** rodapé único com "X ativos selecionados" + um só botão "Adicionar à carteira" agindo sobre os marcados, substituindo o botão por coluna; **(4)** subtítulo classe·emissor sob o nome do ativo no cabeçalho da coluna; **(5)** linhas reordenadas/renomeadas conforme a referência (Taxa, Indexador, Vencimento, Liquidez, Rating, Garantia FGC, Aplicação mínima, Disponibilidade, Concentração pós-aplicação) — "Classe" e "Emissor" saíram da tabela porque passaram a viver no subtítulo do item 4 (sem perda de informação); "Aderência à estratégia" foi mantida com o badge colorido em vez de rebaixar para o texto simples "Suitability" do mockup — mais informativo, mesmo padrão da Fase 9 (badge > texto); **(6)** zebra striping simples (linhas alternando branco/cinza claro) no lugar do destaque pêssego só nas 2 primeiras linhas do mockup, que não correspondia a nenhuma regra identificável (nem sempre a linha "melhor opção"). Cache `ProductsPage.jsx` `20260816b`, `ProductJourney.jsx` `20260816c`.

- **Ajuste pós-Fase 10** ✅ — feedback do usuário sobre a primeira versão: com nenhum cliente ainda escolhido, `ProdCompareInline` escondia a tabela inteira atrás de um estado vazio ("Selecione um cliente para comparar"), obrigando o cliente a ser escolhido antes de ver qualquer coisa. Correção: `ProdCompareView` agora tolera `client == null` — a tabela (checkboxes, Taxa/Indexador/Vencimento/Liquidez/Rating/Garantia FGC/Aplicação mínima/Disponibilidade) fica visível e comparável desde o início; só as 2 linhas que dependem do cliente (Concentração pós-aplicação, Aderência à estratégia — calculadas a partir do saldo/posições/estratégia dele) mostram "Selecione um cliente" no lugar do valor, e o botão "Adicionar à carteira" no rodapé fica desabilitado (com nota "Selecione um cliente para continuar") até haver cliente — comparar não exige cliente, só adicionar à carteira exige. Cache `ProductsPage.jsx` `20260816c`, `ProductJourney.jsx` `20260816d`.

- **Fase 11** ✅ (validada, 0 erros, 12 fluxos de regressão) — usuário reportou que "falta o carrinho, revisar recomendação e confirmar o envio conforme a jornada do Figma". A cadeia Carrinho→Revisar→Confirmar já existia e funcionava ponta a ponta (confirmado por Playwright antes de mexer em qualquer coisa), mas o **carrinho** (`ProdCartBar`, barra horizontal) não tinha nada a ver com a referência: o mesmo nó Figma da Fase 10 tem um frame `carrinho-de-ativos` — painel lateral fixo (ícone + "Carteira" + "N produtos selecionados", lista dos itens com classe/taxa/mínimo, "Total estimado" e botão "Visualizar Carteira") anotado no próprio arquivo como "caso de uso onde o corretor escolhe mais de um ativo e quando ele volta pra página de produtos aparece um carrinho de ativos associado ao cliente". `ProdCartBar` foi substituído por `ProdCartPanel` (`ProductsPage.jsx`): painel `w-80 sticky top-4` ao lado do catálogo (não mais uma barra acima da tabela) — layout de `ProductsPage` virou `flex` (coluna de conteúdo `flex-1` + painel como segundo filho); o nome do cliente não se repete no painel (já está no chip da topbar, Fase 9); "Encerrar carrinho" (sem equivalente no mockup, mas necessário — sem ele o consultor ficaria preso a um carrinho errado) virou um link discreto "Encerrar" no cabeçalho do painel. Também renomeado "% do caixa" → "% carteira" na tabela da Carteira Proposta (mesmo cálculo, só o rótulo, alinhado ao "% CARTEIRA" da referência). **Bug real encontrado ao validar** (pré-existente, não introduzido nesta fase): `ProductJourney.initialViewAndState` mesclava produtos novos vindos de `initialProductIds` só no estado LOCAL (`items`) — nunca avisava o carrinho do `app.jsx` dessa mesclagem; se o consultor saísse com "Continuar buscando" sem tocar em mais nada, o item mesclado sumia silenciosamente do carrinho persistido na próxima abertura. Corrigido com um `useEffect` de montagem que sincroniza (`syncCart`) sempre que a mesclagem inicial adicionou algo novo. Cache `ProductsPage.jsx` `20260816d`, `ProductJourney.jsx` `20260816f`.

- **Ajuste pós-Fase 11** ✅ — usuário mandou um print comparando nossa 1ª versão (painel `sticky` ocupando espaço no layout `flex`, estreitando a tabela e cortando a coluna STATUS) com a referência (painel flutuando por cima do catálogo, sem afetar a largura do conteúdo por baixo). `ProdCartPanel` virou `fixed top-20 right-4 lg:right-6 z-40` (com `max-h-[calc(100vh-6rem)] overflow-y-auto` e `shadow-xl`, sem backdrop — diferente do `Drawer`/`Modal` do Portal, o catálogo por baixo continua visível e clicável) em vez de `sticky` dentro do fluxo; `ProductsPage` voltou a ser uma coluna única (`space-y-4`), já que o painel não reserva mais espaço no layout. Cache `ProductsPage.jsx` `20260816e`.
...
**Fase 12 — alinhamento à referência Figma "Jornada Inter" (2026-08-17, itens de alta prioridade):** análise screen-a-screen das 18 telas do frame Figma "Simulador" (node 39:3) contra a implementação encontrou 11 telas com gaps reais; os 7 de prioridade alta foram implementados e validados (Playwright, 0 erros de console):
1. **Tela de sucesso "Proposta revisada com sucesso!" não existia** — novo passo `aprovada` no wizard (`StepAprovada`, `SimulatorJourney.jsx`), inserido entre Análise e Gerar relatório (`SIM_STEPS`). Mostra card "Resumo da Proposta" (Cliente/Perfil/Valor Total/Produtos/Status) e os botões "Voltar ao perfil do cliente"/"Ver proposta completa". Novo status `aprovada` em `SIMULATION_STATUS_META` (`helpers.js`) — sem ele, a barra de contexto (sempre visível no wizard) continuaria mostrando "Em análise" ao lado do badge "Aprovada" da própria tela.
2. **Dashboard Resumo**: o gráfico central trocou de dois donuts separados para `AllocationCompareBars` (novo, `SimulatorChrome.jsx`) — barras pareadas atual×proposta por classe, incluindo classes a 0%, reaproveitando `AllocationCompareBar` já existente. Adicionado o branch de alerta "Menor liquidez imediata" (antes só existia o caso de melhora); "Diversificação" trocou de rótulo qualitativo (Boa/Média/Baixa) para contagem real de ativos (`ctx.currentEntries.length`/`ctx.proposedEntries.length`). Dashboard ganhou rodapé fixo "Voltar e editar"/"Gerar proposta" (padrão já usado no resto do wizard, mas ausente aqui).
3. **Performance**: nova tabela "Retorno histórico por classe de ativo" (12/24/36M) — a tabela existente comparava carteiras (Atual/Proposta/CDI), não classes. Novo `historicalByClass` em `analytics.js`, mesmo padrão de PRNG semeado do `historicalComparison` existente, aplicado às premissas por classe.
4. **Preview e Compartilhamento**: novo bloco "Resumo executivo" (4 métricas: total investido, retorno proposto/atual em 12M, diferencial em p.p. — mesma métrica da Tela 08) substituindo o antigo bloco de só-texto; adicionada numeração "Pág. N/total" em cada card do preview, dando a sensação de documento paginado sem construir paginação de verdade (o preview continua um scroll único).
5. **Modal de produto incompatível**: removido o botão "Adicionar mesmo assim" (bypass sem fricção do alerta de suitability, que a referência não desenha); botões agora são só "Cancelar"/"Ver alternativas", igual ao mockup — área regulada, sem atalho de contorno.
6. **Alocação excedente**: o alerta de estouro virou um banner de largura total logo abaixo do título (antes era um chip pequeno disputando espaço com outros alertas no card lateral estreito), com o texto completo "R$ X acima do valor da simulação (Y% total). Ajuste os valores abaixo."; botão "Voltar" ganhou o rótulo contextual "Voltar e buscar ativos" quando em estouro.
7. **Busca vazia**: nova seção "Ativos populares sugeridos" (3 chips com botão "Adicionar" cada, produtos fora dos filtros atuais, priorizando elegibilidade e menor risco) como caminho de recuperação — antes só havia "Limpar filtros"/"Cadastrar ativo manualmente".
- **Ajuste pós-implementação**: usuário reportou visualmente que o selo "Pág. 1" na capa do preview (laranja sobre laranja) estava ilegível — `PageBadge` ganhou uma variante `light` para uso sobre o header colorido da capa.
- Cache: `analytics.js`/`SimulatorChrome.jsx`/`SimulatorBuild.jsx`/`SimulatorDashboard.jsx`/`SimulatorJourney.jsx`/`helpers.js` `20260817a`, `SimulatorReport.jsx` `20260817b`.
- **Pendente (prioridade média/baixa)**: gauge de risco (régua) no lugar de `RiskDots`; barras de volatilidade comparativa na Análise técnica; feature real "Selecionar parte da carteira" (Tela 04); coluna de ações na tabela de posições atuais; filtros granulares (CNPJ/subclasse/vencimento) na busca de produtos; ícone de ação da Tela 01 (⋮ vs engrenagem) — não implementados.

**Fase 13 — itens de prioridade média (2026-08-17):** completa a maior parte do que ficou pendente da Fase 12.
- **Tela 02**: cards de resultado de busca ganharam avatar de iniciais; "Clientes recentes" virou lista compacta (ponto colorido por perfil + nome + chevron, sem botão "Selecionar" — clique na linha inteira já seleciona), diferenciando visualmente de um resultado de busca "de verdade".
- **Tela 05**: campo "Vencimento: DD/MM/AAAA" adicionado à linha de cada produto de renda fixa; `ProposalSidePanel` ganhou "Meta da simulação: R$ X" + barra de progresso + contador "Ativos Adicionados (N)" no cabeçalho.
- **Dashboard**: títulos/subtítulos próprios adicionados às abas Risco e Liquidez ("Análise de Risco e Liquidez"), Análise técnica ("Análise Técnica e Estatística") e Comparação ("Compare as carteiras") — antes só existia o título genérico "Análise da proposta" repetido em todas. Risco e Liquidez ganhou badge qualitativo de liquidez ("Alta/Moderada/Baixa liquidez") e a tabela "Período × Atual × Proposta" ao lado das barras; "Maior emissor" passou a citar o nome do emissor, não só o %.
- **Correção de consistência de dado**: a linha "Retorno" da aba Comparação usava `expectedReturn` (premissa anualizada) enquanto a aba Performance usa retorno histórico simulado de 12M para o mesmo conceito — números que pareciam contraditórios entre abas para a mesma proposta. Unificado: Comparação agora usa `analysis.hist.periodReturns` (mesma fonte da Performance) e o rótulo virou "Retorno histórico (12M)".
- **Tela 13**: os 10 rótulos de seção do relatório passaram de abreviados ("Risco × Retorno") para os títulos completos da referência ("Risco x Retorno avançado (Markowitz)", "Matriz de Correlação entre ativos" etc.); card "Identidade" virou "Identidade visual e contato" e passou a mostrar sempre os dados do consultor responsável (antes o toggle trocava a identidade central inteira pelo escritório — agora o toggle só controla se a marca do escritório é aplicada na capa do documento, Tela 14).
- **Tela 14**: campo "Relatório" no card de compartilhamento virou um título descritivo ("Proposta de investimentos — Executiva") em vez de só o nome do formato; corpo do e-mail padrão ganhou uma saudação pré-preenchida quando não há mensagem personalizada configurada.
- **Deliberadamente não implementado** (custo/risco desproporcional ao ganho): "Selecionar parte da carteira" (Tela 04) exigiria estado de seleção threading por todo `proposalContext`, hoje usado por toda a jornada — feature real, não só de forma; coluna "Ações" na tabela de posições atuais (Tela 04) não tem ação real por trás — adicionar um botão decorativo violaria "sem funcionalidade pela metade"; gauge de risco em régua contínua e barras de volatilidade comparativa (Análise técnica) — rework visual de médio porte para itens de prioridade média/baixa; filtros granulares de busca (CNPJ/subclasse/vencimento) e o ícone "⋮" da Tela 01.
- Validado com 2 scripts Playwright novos (`sim-fase12.js` reexecutado + `sim-fase12-medium.js`, 15 checks), 0 erros de console; suíte completa (14 scripts, Simulador + Produtos) sem regressões.
- Cache: `SimulatorBuild.jsx`/`SimulatorDashboard.jsx`/`SimulatorReport.jsx`/`SimulatorJourney.jsx` `20260817c`.
...
**Fase 4 — alinhamento à referência Figma "Jornada Inter" (2026-08-17, itens de alta prioridade):** análise das 14 telas do frame Figma "Planejamento" (node `90:989`) contra o app rodando de verdade (servidor local + Playwright headless) encontrou 4 bugs reais, só visíveis em execução (não apareciam na leitura estática do Figma). Todos corrigidos e validados (0 erros de console):
1. **Tabelas editáveis sem formatação de moeda** — `PlanEditableTable` ganhou `column.type === 'currency'` (mantendo `'number'` puro para colunas não-monetárias como Idade): valor formatado (`R$ 32.000`, via `formatCurrency` já existente) fora de foco, número cru editável em foco. Aplicado às 3 tabelas de valor: Rendas/Despesas, Investimentos financeiros, Outros bens.
2. **Truncamento de texto nas colunas** — `PlanEditableTable` ganhou `min-width` por tipo de coluna + `title` (tooltip nativo) nas colunas de texto; "Indeterminado"/"Financiamento" deixaram de cortar.
3. **Bug de dados corrigido**: `financialPlans[0].assumptions.validated` (seed, `mock-data.js`) gravava chaves (`inflation/realReturn/nominalReturn`) diferentes das lidas pela tela (`taxa/inflacao/despesas/risco`) — as 4 checkboxes de "Premissas validadas pelo consultor" nunca apareciam marcadas, independente do seed. Chaves corrigidas e as 4 marcadas como `true` (igual ao Figma).
4. **Separador decimal BR** nos 6 campos de premissas (Inflação/Retorno nominal/Retorno real/CDI/Crescimento despesas/Crescimento renda) — trocaram de `<input type="number">` (mostrava `4.5`) para `<input type="text" inputMode="decimal">` com edição em vírgula (`4,5`), texto cru em foco e commit só com número válido.
- Cache: `PlanningTab.jsx` `20260817a`, `mock-data.js` `20260817b`.

**Fase 5 — dividir a Tela 1 do wizard em duas telas, conforme o Figma (2026-08-17):** o Figma tem 2 telas separadas para o passo 1 ("Iniciar planejamento" e "Perfil familiar"); a implementação as tinha fundido em uma só (`PlanStepContexto`) e não tinha o painel "Impacto familiar no plano". Corrigido:
- `PlanStepContexto` dividido em `PlanStepNovo` (nome/tipo/horizonte/observações + "O que será construído") e `PlanStepPerfilFamiliar` (dados pessoais/dependentes/contexto adicional + novo painel **"Impacto familiar no plano"**, calculado a partir de `draft.context.dependents`: contagem de dependentes, anos de pressão de caixa até os 24 anos do dependente mais velho, nota sobre proteção de renda).
- `PlanAttention` ganhou uma prop `title` opcional (default `"Requer atenção"`, sem quebrar os usos existentes) para ser reaproveitado pelo novo painel — sem novo primitivo visual.
- `WIZARD_STEPS` ganhou o item `perfil` (array de 6 → 7); `PlanningWizard` reindexado (`last = step === 5`), `PlanStepper` já é 100% data-driven, sem mudanças.
- Cache: `PlanningTab.jsx` `20260817c`.

**Fase 6 — entrada tool-first "Planejamento → Cliente" (2026-08-17):** fecha a lacuna registrada em `GOVERNANCA.md:249` (mirror do "Nova simulação" do Simulador). Diferente de simulações (múltiplas por cliente, `clientId` nulável), planos são 1:1 por cliente — o desenho não replicou a tela de seleção própria do Simulador (`StepCliente`); seguiu o Figma, que embute um campo **"Associar a um cliente"** na própria Tela 1 do wizard, sem tela dedicada.
- Novo item de rail **"Planejamento"** (`Portal/components/Shell.jsx`, `permKey: 'planning'`, ícone `flag`) entre Produtos e Recomendações; liberado para os perfis comerciais (consultor/alocador/gestor/admin — mesmo conjunto de `products`/`recommendations`), oculto para Daily Banker e o perfil sem vínculo.
- Novo branch `page === 'planejamento'` em `app.jsx` renderiza `window.PlanningWizard` **sem** o header/tabs do cliente (só sidebar + topbar do Shell), igual ao frame Figma `planejamento-novo-sem-cliente`.
- `PlanStepNovo` ganhou o combobox `PlanClientPicker` (busca por nome/CPF, reaproveitando o padrão de `StepCliente` do Simulador como referência de UX, componente próprio); `PlanningWizard` aceita `client=null` + `clients`, mantém o cliente escolhido em estado local, e bloqueia "Salvar rascunho"/"Calcular plano" (com hint inline) até haver cliente associado.
- **Guarda-corpo de integridade**: como plano é 1:1 por cliente, `PlanClientPicker` desabilita (com "Já possui plano") clientes que já têm um plano — evita que a entrada tool-first sobrescreva silenciosamente um plano existente (ex.: Mariana) com um rascunho em branco. `app.jsx` computa a flag via `clientsWithPlanFlag` (memo sobre `financialPlans`).
- Commit reaproveita o **mesmo** `commitPlanDraft(clientId, draft, status)` do fluxo client-first (nenhum handler novo); ao salvar, `openClient(clientId, { tab: 'planning' })` (novo 2º parâmetro opcional, retrocompatível) aterrissa na aba Planejamento já existente do cliente escolhido. `ClientProfilePage` ganhou `initialTab` (default `'overview'`, retrocompatível) para isso.
- **Bug real encontrado ao validar** (pré-existente, não introduzido nesta fase, mas só alcançável agora com facilidade): `PlanOverview`, `PlanResult`, `PlanScenarios`, `PlanComparison`, `PlanRecommendations` e `PlanTransition` acessavam `plan.scenarios[0]`/`[1]` sem guarda — qualquer plano novo (`scenarios: []` até hoje só populado à mão no seed da Mariana) quebrava com `TypeError` ao abrir a Visão geral. Corrigido com fallback `|| {}` nos 6 pontos de acesso; os campos derivados (aporte, idade, etc.) ficam vazios/zerados em vez de quebrar a tela — gerar cenários reais para clientes sem seed continua fora do escopo (não há motor de cálculo genérico, só os números fixos da Mariana).
- Validado com Playwright (servidor local, 0 erros de console): navegação pelo rail, tela sem cliente, busca e seleção de cliente, guarda-corpo para cliente com plano existente, wizard completo até "Calcular plano", aterrissagem correta na ficha do cliente, e regressão completa do fluxo client-first original (Mariana) sem alterações de comportamento.
- Cache: `PlanningTab.jsx` `20260817f`, `Shell.jsx` `20260817a`, `ClientProfilePage.jsx` `20260817a`, `mock-data.js` `20260817c`, `app.jsx` `20260817b`.

**Fase 7 — polish (2026-08-17):** `PlanTransition` ganhou o indicador "1. Planejamento → 2. Simulador" (Definição de Metas & Gap / Estruturação da Carteira) que existe no frame Figma `conexao-simulador`, reaproveitando o mesmo padrão de círculo numerado usado no `PlanStepper`. Cache: `PlanningTab.jsx` `20260817g`. Com isso, os 4 achados de alta prioridade, a divergência estrutural, a lacuna arquitetural e o item de polish do levantamento Figma inicial estão todos fechados.

**Ajuste pós-Fase 7 — menu final do rail (2026-08-17):** usuário definiu o menu lateral final: **Visão geral, Clientes, Produtos, Simulador, Planejamento, Ordens, Relatórios (em breve), Operações, Suporte**. O item antes rotulado "Recomendações" (rota `simulacoes`, já é a jornada do Simulador por decisão de arquitetura antiga) virou **"Simulador"** (mesma rota/`permKey: 'recommendations'`, só renomeado — alinha com o breadcrumb da própria página, que já dizia "Investimentos / Simulador"), reordenado para ficar antes de "Planejamento". Nenhum item "Recomendações" isolado sobrou no rail. Cache: `Shell.jsx` `20260817b`.
...
### Jornada de Operações (2026-08-18) — reconstrução completa do módulo

O item de menu "Operações" existia como uma tela simples (tabela de "solicitações de serviço" com 3 status). A referência Figma "Jornada Inter" (frame "Jornada operações - home e abas", node `280:29588`) desenha um módulo bem mais rico e autônomo, com **5 abas**: Minha fila, Todas, Em risco, Concluídas e Dashboard. Reconstruído do zero em [Portal/pages/OperationsPage.jsx](Portal/pages/OperationsPage.jsx), entregue em 2 partes e validado com Playwright (0 erros de console) em ambas.

- **Decisão de escopo**: o modelo antigo (`serviceRequests`/`ServiceRequestDrawer`) segue existindo **intocado** — ele alimenta a aba Banking da ficha do cliente (US-14), fora do escopo deste Figma. O novo módulo usa um dataset próprio, `DATA.operations` (`Portal/data/mock-data.js`, ~40 registros distribuídos entre os 17 clientes existentes), com um modelo mais rico: `priority`, `status` (9 estados, de "Novo" a "Concluído"/"Concluída parcialmente"), `responsavel` (operador interno, não o consultor logado), `openedAt`/`dueAt`/`resolvedAt`, `motivoPrincipal`, `nextAction`, `timeline`. Novos dicionários/helpers em `Portal/lib/helpers.js`: `OPERATION_TYPE_META`, `OPERATION_PRIORITY_META`, `OPERATION_STATUS_META`, `OPERATION_STATUS_FLOW` (sequência usada por "Avançar status"), `OPERATION_RESULT_META`, `durationLabel(ms)` e `operationSlaState(op, now)`/`operationSlaResult(op)` (limiar de risco: SLA ≤ 3h).
- **Escopo por perfil**: reaproveita exatamente o padrão já usado por `scopedServiceRequests`/`scopedOrders` (`scopedClientIds` derivado de `clientsInScope`) — `scopedOperations` novo em `Portal/app.jsx`, sem lógica de escopo nova.
- **Minha fila / Todas / Em risco**: mesma `OpsQueueTable`, parametrizada por `scope`. "Minha fila" = operações abertas no escopo; "Todas" = escopo completo (abertas + concluídas); "Em risco" = abertas com SLA em risco/vencido. KPIs (Na minha fila/Aguardando terceiros/Em risco de SLA/SLA vencido/Concluídas hoje) e banner de urgência calculados ao vivo sobre o dataset, não fixos como no mockup Figma (que usa números de exemplo avulsos). Filtros (Prioridade/Status/Tipo/Responsável + busca por cliente/protocolo) como `<select>`s, no mesmo padrão de `OrdersPage`/`ProductsPage` — não os chips-com-chevron do Figma, para não introduzir um segundo padrão visual de filtro. **Paginação nova** (`Mostrando X-Y de Z` + Anterior/Próxima): não existia esse padrão em nenhuma outra tela do protótipo; implementada client-side, 8 itens/página.
- **Concluídas**: KPIs de resultado (Concluídas/Dentro do SLA/Tempo médio/Fora do SLA), bloco "Principais causas de atraso" (barras de %, agregadas de `motivoPrincipal`), tabela com Resultado/Tempo total/SLA/Motivo — tudo derivado do dataset.
- **Dashboard**: filtros (Responsável/Tipo/Prioridade/Segmento — "Equipe" e um seletor de "Período" funcional do Figma foram deliberadamente deixados de fora: não existe conceito de equipe no modelo de dados, e um seletor de período que não filtra de verdade seria pior que não ter um), 6 KPIs, barra segmentada "Operações por status" (novo, `OpsSegmentedBar`), donut "Cumprimento de SLA" + lista "SLA por tipo de operação" (`ChartCanvas` doughnut direto — **não** reaproveita `AllocationDonut`/`classHex`, que mapeia cor por classe de ativo e não serve para rótulos livres), "Aging das operações abertas" (`OpsAgingList`, 5 baldes), "Principais motivos de espera" e "Carga por responsável" (tabelas agregadas), "Precisam de atenção agora" (top 5 urgentes, "Abrir caso" abre o drawer), e o gráfico "Operações abertas x concluídas" — série diária pelo intervalo real dos dados (o protótipo tem ~5 dias de dados, não 30; o rótulo do período reflete o intervalo real em vez de um "Últimos 30 dias" fixo que não bateria com o gráfico).
- **`OperationDrawer`** (painel lateral usado nesta primeira entrega): protocolo/cliente/prioridade/status/responsável/SLA/aging/motivo/próxima ação + timeline; ação "Avançar status" percorria `OPERATION_STATUS_FLOW`. **Substituído na entrega seguinte** por uma página cheia de detalhe (ver abaixo) — mantido aqui só como registro histórico.
- **Bug corrigido durante a validação**: o cabeçalho da aba Dashboard mostrava uma contagem numérica antes do subtítulo ("19 visão gerencial e analítica das demandas") — a mesma composição `{contagem} {subtítulo}` usada nas outras 4 abas não fazia sentido para o Dashboard, cujo subtítulo já é uma frase completa. Corrigido: só as abas de lista (fila/todas/risco/concluídas) prefixam a contagem.
- **Bug corrigido durante a validação**: o rodapé "Protótipo 100% mockado…" estava duplicado na aba Dashboard — a página já herda esse aviso do `Shell` (global, em toda tela); um segundo `<p>` igual tinha sido copiado dos outros dashboards da sessão (que não têm esse problema porque não duplicam o aviso). Removido do `OperationsPage`.
- Validado com 2 scripts Playwright (fila/todas/risco/concluídas + drawer + paginação + filtros; dashboard completo + gráficos + filtros), 0 erros de console, testado em 2 perfis (`consultor`, escopo próprio, e `admin`, escopo `all`) para garantir que os números batem em qualquer tamanho de carteira. Regressão: navegação completa por todas as páginas do rail + fluxo antigo da aba Banking (US-14), 0 erros novos.
- Cache: `mock-data.js`/`helpers.js` `20260818a`, `OperationsPage.jsx` `20260818b`, `app.jsx` `20260818a`.

**Detalhe da operação — página cheia (2026-08-18):** a referência Figma "Jornada operações - alteração cadastral" (node `280:29597`, 14 telas + 4 estados de exceção) desenha um **case-management workspace completo** para uma operação — não um drawer, uma página própria (`← Voltar para Operações`, protocolo/SLA/Reatribuir/Escalar no cabeçalho, 3 abas: Visão geral/Documentos/Histórico). O Figma ilustra só o tipo "Alteração cadastral"; a implementação generaliza o mesmo template para os 9 tipos de operação. Nova página [Portal/pages/OperationDetailPage.jsx](Portal/pages/OperationDetailPage.jsx), roteada via `page === 'operationDetail'` (`pageParams.operationId`, mesmo padrão de `pageParams.simulationId` do Simulador) — **substitui** o `OperationDrawer` da entrega anterior: clicar numa linha da fila/concluídas/dashboard agora navega para esta página em vez de abrir um painel lateral.
- **Aditivo, sem risco ao que já estava no ar**: nenhum campo existente de `DATA.operations` foi renomeado/removido; os campos novos (`documents`, `messages`, `resumoResolucao`) têm fallback (`|| []`/ausente) e não exigiam migração dos ~40 registros já seedados. 3 status novos e aditivos em `OPERATION_STATUS_META` (`executada`, `nao_executada`, `cancelada`) — nenhum registro existente os usava antes; só são alcançados a partir de agora, pelas novas ações.
- **Progresso da solicitação** (stepper de 6 estágios — Criada/Triagem/Validação/Documentos/Execução/Concluído): **derivado do `status`** via `operationStageIndex` (novo helper), não é um campo próprio — evita 2 fontes de verdade que pudessem divergir.
- **Ações por status** (genéricas, funcionam para qualquer um dos 9 tipos): Novo→"Iniciar triagem"; Em análise→"Concluir triagem"; Aguardando documento/consultor→"Solicitar"/"Marcar como recebido"/"Abrir conversa"; Pendência interna→"Validar documento" (abre drawer com checklist de 4 critérios; documento inválido reabre o ciclo em "Aguardando documento", documento válido avança para "Em processamento" — mesmo mecanismo de correção em loop do Figma); Em processamento→"Executar operação" (modal de confirmação); Aguardando backoffice→"Confirmar execução do backoffice"; Executada→"Concluir operação" (modal rico: resultado entre 4 opções, resumo, motivo/causa, notificações). "Reatribuir" e "Escalar" (motivo + observação + destino, sobe prioridade para Crítica) disponíveis em qualquer estado aberto.
- **"Abrir conversa"**: drawer com thread de mensagens (`operation.messages`, novo campo) — funcional (envia e persiste na sessão), não um placeholder; propositalmente simplificado (sem anexos/typing indicator).
- **Detalhes do SLA**: "Meta total"/"Tempo consumido"/"Tempo restante" **derivados** de `openedAt`/`dueAt`/`now` (não um orçamento fixo "8h" como no mockup Figma) — mesma disciplina de "número computado, não hardcoded" das outras abas do módulo.
- **Deliberadamente simplificado**: pré-visualização de documento é um card estático (sem render de PDF real); "Solicitada por" é derivado do consultor-dono do cliente (`OWNER_NAME_MAP[client.ownerId]`) em vez de um campo próprio, já que o dado já existe e evita adicionar mais um campo a 40 registros.
- Validado com 5 scripts Playwright cobrindo: navegação completa da página + as 3 abas; ciclo de vida completo aguardando_documento→pendência_interna→em_processamento→executada→concluída (com os modais de validar/executar/concluir); os 3 ramos não cobertos pelo ciclo principal (novo, em_análise, aguardando_backoffice); Reatribuir, Escalar (+ bump de prioridade) e Abrir conversa (+ envio de mensagem); abertura read-only de uma operação já concluída (sem stepper/pendências/ações). 0 erros de console em todos. Regressão completa das 5 abas da fila/dashboard e do fluxo antigo (Banking/US-14), 0 erros novos.
- Cache: `helpers.js` `20260818b`, `OperationsPage.jsx` `20260818c`, `OperationDetailPage.jsx` `20260818a` (novo), `app.jsx` `20260818b`.

### Jornada Admin (2026-08-18) — módulo de Administração

Referência Figma "Jornada Admin" (node `281:30245`) traz **2 entregas distintas** sob o mesmo frame: (1) uma redesenho da "Visão geral" (Home) com KPIs/gráfico/alertas novos, e (2) o módulo **Administração** propriamente dito (11 telas: dashboard de contexto + usuários/acessos, detalhe de usuário, perfis/permissões, detalhe de perfil, gestão de carteiras, carteira de 1 consultor, histórico, novo usuário, transferir cliente, transferir em massa). **Só a (2) foi implementada** — a redesenho da Home é uma mudança de maior risco (toca a página de entrada já em produção, usada por todo perfil) e ficou deliberadamente fora desta entrega; ver "Débitos técnicos" abaixo.

- **Novo item de rail "Administração"** (`Portal/components/Shell.jsx`, ícone `shield`, `permKey: 'admin'`) — visível para todo perfil (mesmo padrão "trancado" de "Relatórios"), mas só o perfil Administrador (Bianca Rocha) tem `admin` em `permissions.menu`; os demais veem o item cinza com cadeado e a página mostra `NoPermissionState` se acessada.
- **Novo dataset independente de `profiles`**: `DATA.portalUsers` (`Portal/data/mock-data.js`, 15 colaboradores fictícios — Administrador/Gestor/Consultor/Daily Banker/Backoffice) e `DATA.adminAuditLog` (trilha de auditoria, cresce em runtime). `profiles` (os 6 cenários do seletor no rodapé do rail) **não foi tocado** — são conceitos diferentes: `profiles` simula "logar como", `portalUsers` é o quadro real de colaboradores gerenciado pela tela. Dois dos novos usuários (Rafael Almeida, Renato Vieira) nascem **sem carteira** (`ownerId` próprio, zero clientes vinculados) — de propósito, para exercitar o estado vazio e servir de destino nas transferências de teste.
- **Mudança arquitetural**: `clients` virou **estado** em `Portal/app.jsx` (`useState(DATA.clients)`, antes era lido direto de `DATA.clients` em 10 pontos, todos migrados) — necessário porque "Transferir cliente(s)" reatribui `client.ownerId`, o mesmo campo que `clientsInScope` usa em **todo o app** (Clientes, Ordens, Operações, Simulador, Planejamento). A transferência é real e propaga: um cliente transferido some da lista de Clientes do consultor de origem e aparece na do destino imediatamente (validado por Playwright, perfil trocado no meio do teste).
- **Perfis e permissões**: `ROLE_DEFINITIONS` (novo, `helpers.js`) é uma matriz **decorativa/informativa** (Visualizar/Criar/Alterar/Executar por 6 funcionalidades, por perfil) — deliberadamente **não** ligada a `profiles[].permissions` (o motor real de escopo do protótipo); são as "vitrines" que o Figma desenha, não o mecanismo de fato. "Editar perfil" do mockup **não foi implementado**: edição de verdade exigiria promover a matriz a estado, e um botão decorativo sem efeito violaria a diretriz de não ter funcionalidade pela metade.
- **Gestão de carteiras**: linhas = usuários com `ownerId` (consultores/daily bankers reais); todas as métricas (clientes/patrimônio/segmento principal/operações abertas/recomendações/pendências) **computadas ao vivo** a partir de `clients`/`orders`/`recommendations`/`operations`, sem números fixos.
- **Transferir cliente** (1) e **Transferir em massa** (N): dois modais distintos (o Figma desenha telas diferentes — o de massa tem "simulação pós-transferência" antes/depois e aviso de excesso de capacidade, o de 1 não). Ambos com rádio "manter/transferir responsabilidade das operações abertas" — quando "transferir", `Operações` ganhas do novo consultor têm `responsavel` atualizado. Toda transferência grava uma linha em `adminAuditLog`.
- **Bug real encontrado e corrigido durante a validação**: ao confirmar uma transferência, `AdminBookDetailPage` limpava a seleção (`setSelected([])`) **antes** do modal fechar; como as props do modal derivavam de `selectedClients` (recalculado a cada render a partir da seleção), o próximo render tentava ler `selectedClients[0].id` com a lista já vazia → `TypeError`. Corrigido capturando os clientes selecionados em `transferTarget.clientsSelected` no momento de abrir o modal (snapshot, não mais derivado ao vivo) e só limpando a seleção quando o modal fecha.
- **Bug menor corrigido**: "Tempo na carteira" sempre pluralizava ("1 anos"); corrigido para singular quando o valor é 1.
- **Novo usuário**: wizard de 4 etapas (Dados/Permissões/Carteira/Revisão) — permissões granulares agrupadas (`NEW_USER_PERMISSION_GROUPS`, `helpers.js`), com a mesma confirmação extra do Figma para permissões sensíveis ("Confirmo que este usuário deve possuir essa permissão").
- Validado com 4 scripts Playwright: as 4 abas do hub + KPIs; detalhe de usuário (bloquear/desbloquear, abas incluindo Carteira) + navegação para a carteira; transferência única e em massa (incluindo o bug acima, achado e corrigido no processo) + wizard completo de novo usuário; efeito cross-app de uma transferência real (cliente sai da lista de um consultor e aparece pronto para o outro, trocando de perfil no meio do teste). 0 erros de console em todos. Regressão completa: Simulador/Operações/Produtos/Planejamento/Ordens/Suporte + fluxo antigo Banking (US-14), 0 erros novos.
- Cache: `mock-data.js` `20260818b`, `helpers.js` `20260818c`, `Shell.jsx` `20260818a`, `app.jsx` `20260818c`, `AdminPage.jsx` `20260818a` (novo), `AdminDetailPages.jsx` `20260818c` (novo).

---

## Débitos técnicos e decisões pendentes (não são épicos, mas afetam qualquer evolução)

- **Redesenho da "Visão geral" (Home) — pendente.** O frame Figma "Jornada Admin" (node `281:29902`, "dashboard-inter") traz uma versão nova da Home (KPIs de captação/retirada, gráfico de 7 dias, alertas com dropdown de status inline, "Atalhos", "Top clientes com saldo disponível") — mais rica que a `HomePage.jsx` atual. Deliberadamente **não implementado** junto com o módulo Administração (2026-08-18): é uma mudança de maior risco por tocar a página de entrada já em produção, usada por todos os perfis — merece sua própria análise/implementação, não ir de carona numa entrega de outro módulo.
