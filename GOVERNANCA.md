...
Novo épico (detalhado em [Portal/EPICO_PORTAL_CONSULTOR.md](Portal/EPICO_PORTAL_CONSULTOR.md#ep-02--jornada-de-produtos-e-carteira-proposta)): a **continuação operacional** do Simulador. O Simulador define a **carteira-alvo** (classes/percentuais); esta jornada seleciona os **ativos reais** que a implementam e gera uma **recomendação executável (#REC)** rastreável em Ordens. Decisão de arquitetura **Opção A**: o Simulador **não** é refatorado (mantém sua seleção de produtos legada) — ganha só uma ponte "Implementar em Produtos"; o cleanup da sobreposição fica para depois. Cliente-vitrine **João Pedro Silva (C16)** com `targetAllocation` semeado (padrão da Mariana no Planejamento). Componentes internos com prefixo `Prod*` (escopo global compartilhado). Entregue em 3 fases:
...
- **Fase 2** ⏳ — Tela 03 (Comparar), Tela 04 (Detalhe + RateInput + Impacto na estratégia), multi-select/barra contextual.
- **Fase 3** ⏳ — Tela 05 (Carteira proposta), 06 (Revisar), 07 (Confirmar), 08 (Ordens #REC expansível) + estados de erro (incl. saldo excedido bloqueante).
